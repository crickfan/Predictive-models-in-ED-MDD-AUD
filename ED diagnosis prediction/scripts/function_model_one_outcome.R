...
if (mod == "no-BMI") # exclude BMI
{
    ftNames <- with(dfModality, na.omit(VarName[ToClassify == 1 & Category != "BMI"]))

    rawData <- subset(rawData, select = ftNames)
    
} else if (mod == "all") { # else use all features
  ftNames <- with(dfModality, na.omit(VarName[ToClassify == 1]))
  rawData <- subset(rawData, select = ftNames)
  
} else { # use specific modality, add covar
  ftNames <- with(dfModality, na.omit(VarName[Category == mod & ToClassify == 1]))
  # add covar to modality specific analysis
  ftNames <- c(ftNames, coVars)
...
for (r in 1:Repeats)
{
  ...
  for (f in 1:OuterCVFolds)
  {
    ...
    ##trainWeights = sampleWeights[trainIdx]
...
meanImp <- meanImp[order(abs(meanImp$median_t)),]
...
#4.1 save predicted values and outcome
